...
int main() {
	...
	//char* array[] = { "V4.6","email-Eu-core","label_4", "new04_1" };
	char* array[] = { "V4.6","","", "ex4_2" };
...
void ETOF_KGPM(char** array)
{
	...
	init();
	list<int>::iterator cnode;

	for (cnode = nodeCandidate[1].begin(); cnode != nodeCandidate[1].end(); cnode++) {
		bool sflag = TopologicalMatching3(*cnode,1);
