...
struct NodeDist {	// 该数据结构用于存储A到其他节点的距离
	SUITABLE_PATH suitable_path;
	vector<ArcNode> path;
	bool finishFlag; // 用于标识当前节点是否已被Dijkstra算法选择过
	double Utility;
	double allRou;
	NodeDist() {
		finishFlag = false;
...
//void Dijkstra_Algo(int starti, PArcNode *p, map<int, SUITABLE_PATH> edgeCandidate, int direction);
