...
int edge_match(int starti, int num, PArcNode *p, vector<int> *connected_node_list, int existFlag) {//遍历数据图,对数据图中标签与模式边的起始节点相同的节点进行有届深度搜索
	...
	int direction = 0; // 正向边匹配
	Dijkstra_Algo_0(starti, p, &edgeCandidate);
...
int inverse_edge_match(int endi, int num, PArcNode *p, vector<int> *connected_node_list, int existFlag) {
	...
	int direction = 1; // 反向边匹配
	//Dijkstra_Algo(endi, p, edgeCandidate, direction);
	Dijkstra_Algo_1(endi, p, &edgeCandidate);
...
*  用于正向边匹配的Dijkstra算法
*/
void Dijkstra_Algo_0(int starti, PArcNode *p, map<int, SUITABLE_PATH> *edgeCandidate) {
	vector<NodeDist> notFound;
	// 初始化 notFound;
	for (int i = 0; i < G.vexnum; i++) {
		NodeDist nodeDisti;
		notFound.push_back(nodeDisti);
	}

	// 初始化从开始节点开始的最短路径
	ArcNode *startnodeArc = G.NodeList[starti].firstoutarc;
	while (startnodeArc != NULL) {
		//计算路径长度并存储
		NodeDist nodeDistj;
		nodeDistj.path.push_back((*startnodeArc));

		nodeDistj.suitable_path.startnode = starti;
		nodeDistj.suitable_path.endnode = (*startnodeArc).adjvex;
		nodeDistj.suitable_path.path_list.push_back(starti);
		nodeDistj.suitable_path.path_list.push_back((*startnodeArc).adjvex);

		nodeDistj.allRou = G.NodeList[starti].factor + G.NodeList[(*startnodeArc).adjvex].factor;
		nodeDistj.suitable_path.t = (*startnodeArc).trust;
		nodeDistj.suitable_path.r = (*startnodeArc).intimacy;
		//nodeDistj.suitable_path.rou = nodeDistj.allRou / (nodeDistj.path.size() + 1);
		
		nodeDistj.Utility = max(max((*p).pintimacy / nodeDistj.suitable_path.r, (*p).ptrust / nodeDistj.suitable_path.t),  nodeDistj.path.size()/(*p).boundlength);

		notFound[(*startnodeArc).adjvex] = nodeDistj;
		startnodeArc = startnodeArc->nextarc;
	}
	...
	for (int i = 0; i < G.vexnum; i++) {
		...
		for (int j = 0; j < G.vexnum; j++) {
			if (notFound[j].Utility>0 && notFound[j].finishFlag==false) {
				if (notFound[j].Utility - minUti < 0) {
					minUti = notFound[j].Utility;
					minNo = notFound[j].suitable_path.endnode;
				}
			}
		}
		// 剪枝，提前结束
		...
		//else if(notFound[minNo].path.size()>(*p).boundlength) {
		//	notFound[minNo].finishFlag = true;
		//	continue;
		//}
		//if (minNo == -1) {
		//	return;
		//}
		if (G.NodeList[notFound[minNo].suitable_path.endnode].nodelable == P.pnodeList[(*p).toNode].PNodeLable &&
			(G.NodeList[notFound[minNo].suitable_path.endnode].factor - P.pnodeList[(*p).toNode].factor>-0.000001)) {
			// 将满足条件的路径加到匹配路径列表中
			notFound[minNo].suitable_path.rou = notFound[minNo].allRou / (notFound[minNo].path.size() + 1);
			if (notFound[minNo].suitable_path.rou - (*p).pfactor>-0.0000001) {
				(*edgeCandidate)[notFound[minNo].suitable_path.endnode] = notFound[minNo].suitable_path;
			}			
		}
		...
		notFound[minNo].finishFlag = true;
		// 遍历刚刚找到的距离最短节点的邻接点
		ArcNode *arcTemp = G.NodeList[minNo].firstoutarc;
		while (arcTemp != NULL) {
			//计算路径长度并存储
			NodeDist nodeDistk;
			nodeDistk = notFound[minNo];
			nodeDistk.finishFlag = false;
			nodeDistk.path.push_back((*arcTemp));
			nodeDistk.suitable_path.endnode = (*arcTemp).adjvex;
			nodeDistk.suitable_path.path_list.push_back((*arcTemp).adjvex);

			nodeDistk.allRou = nodeDistk.allRou + G.NodeList[(*arcTemp).adjvex].factor;
			nodeDistk.suitable_path.t = nodeDistk.suitable_path.t * (*arcTemp).trust;
			nodeDistk.suitable_path.r = nodeDistk.suitable_path.r * (*arcTemp).intimacy;
			//nodeDistk.suitable_path.rou = nodeDistk.allRou / (nodeDistk.path.size() + 1);

			nodeDistk.Utility = max(max((*p).pintimacy / nodeDistk.suitable_path.r, (*p).ptrust / nodeDistk.suitable_path.t), nodeDistk.path.size() / (*p).boundlength);
			if (notFound[(*arcTemp).adjvex].Utility>0) {
				if (nodeDistk.Utility - notFound[(*arcTemp).adjvex].Utility < 0) {
					notFound[(*arcTemp).adjvex] = nodeDistk;
				}
			}
			else {
				notFound[(*arcTemp).adjvex] = nodeDistk;
...
*  用于反向边匹配的Dijkstra算法
*/
void Dijkstra_Algo_1(int endi, PArcNode *p, map<int, SUITABLE_PATH> *edgeCandidate) {
	vector<NodeDist> notFound;
	// 初始化 notFound;
	for (int i = 0; i < G.vexnum; i++) {
		NodeDist nodeDisti;
	...
	// 初始化从开始节点开始的最短路径
	ArcNode *endnodeArc = G.NodeList[endi].firstinarc;
	while (endnodeArc != NULL) {
		//计算路径长度并存储
		NodeDist nodeDistj;
		nodeDistj.path.push_back((*endnodeArc));

		nodeDistj.suitable_path.startnode = (*endnodeArc).adjvex; 
		nodeDistj.suitable_path.endnode = endi;
		nodeDistj.suitable_path.path_list.push_back(endi);
		nodeDistj.suitable_path.path_list.push_back((*endnodeArc).adjvex);

		nodeDistj.allRou = G.NodeList[endi].factor + G.NodeList[(*endnodeArc).adjvex].factor;
		nodeDistj.suitable_path.t = (*endnodeArc).trust;
		nodeDistj.suitable_path.r = (*endnodeArc).intimacy;
		//nodeDistj.suitable_path.rou = nodeDistj.allRou / (nodeDistj.path.size() + 1);

		nodeDistj.Utility = max(max((*p).pintimacy / nodeDistj.suitable_path.r, (*p).ptrust / nodeDistj.suitable_path.t), nodeDistj.path.size() / (*p).boundlength);

		notFound[(*endnodeArc).adjvex] = nodeDistj;
		endnodeArc = endnodeArc->nextarc;
	}
	...
	for (int i = 0; i < G.vexnum; i++) {
		...
		for (int j = 0; j < G.vexnum; j++) {
			if (notFound[j].Utility>0 && notFound[j].finishFlag == false) {
				if (notFound[j].Utility - minUti < 0) {
					minUti = notFound[j].Utility;
					minNo = notFound[j].suitable_path.startnode;
				}
			}
		}
		//// 剪枝，提前结束
		...
		//	return;
		//}
		if (G.NodeList[notFound[minNo].suitable_path.startnode].nodelable == P.pnodeList[(*p).fromNode].PNodeLable &&(G.NodeList[notFound[minNo].suitable_path.startnode].factor - P.pnodeList[(*p).fromNode].factor>- 0.000001)) {
			// 将满足条件的路径加到匹配路径列表中
			notFound[minNo].suitable_path.rou = notFound[minNo].allRou / (notFound[minNo].path.size() + 1);
			if (notFound[minNo].suitable_path.rou - (*p).pfactor>-0.0000001) {
				(*edgeCandidate)[notFound[minNo].suitable_path.startnode] = notFound[minNo].suitable_path;
			}
			//(*edgeCandidate)[notFound[minNo].suitable_path.startnode] = notFound[minNo].suitable_path;
		}
		// 
		...
		notFound[minNo].finishFlag = true;
		// 遍历刚刚找到的距离最短节点的邻接点
		ArcNode *arcTemp = G.NodeList[minNo].firstinarc;
		while (arcTemp != NULL) {
			//计算路径长度并存储
			NodeDist nodeDistk;
			nodeDistk = notFound[minNo];
			nodeDistk.finishFlag = false;
			nodeDistk.path.push_back((*arcTemp));
			nodeDistk.suitable_path.startnode = (*arcTemp).adjvex;
			nodeDistk.suitable_path.path_list.push_back((*arcTemp).adjvex);
